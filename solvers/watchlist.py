...
def setup_watchlist(instance):
    watchlist = [deque() for __ in range(len(instance.variables))]

    for clause in instance.clauses:
        # Make the clause watch its first literal
        # print("\nclause: {}".format(clause))
        # print("\nclause[0]: {}".format(clause[0]))
        # print("\nvariable_table[abs(clause[0])]: {}".format(instance.variable_table[abs(clause[0])]))

        # e.g : clause: (-112,-111)  || variable_table: {111: 0, 112: 1, 113: 2, ...}
        # -> instance.variable_table[abs[clause[0]]]: 1 
        vt_index = instance.variable_table[abs(clause[0])]
        
        watchlist[vt_index].append(clause)
    return watchlist
